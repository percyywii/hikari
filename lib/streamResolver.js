...
export async function resolveStreamingSources({
  ...
  try {
    ...
    const providerAttempts = [reanimeAttempt, senshiAttempt];

    while (providerAttempts.length) {
      const settled = await withResolutionTimeout(Promise.race(
        providerAttempts.map((attempt, index) => attempt.then((value) => ({ index, ...value }))),
      ));
      if (resolutionDeadline.signal.aborted) throw resolutionDeadline.signal.reason;
      providerAttempts.splice(settled.index, 1);
      ...
      if (settled.result?.sources?.length) {
        ...
        if (validated.sources.length > 0 || embedSources.length > 0) {
          const result = {
            ...settled.result,
            sources: directSources.length === 0 ? embedSources : [...validated.sources, ...embedSources],
          };
        ...
        const reason = validated.failures?.[0]?.reason || "invalid source response";
        errors.push(`${settled.provider}: ${reason}`);
        logDevelopmentWarning(settled.provider, reason);
        continue;
      }
